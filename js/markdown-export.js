    ...
    try {
        ...
        data.stages.forEach((stage, index) => {
            ...
            stage.processes.forEach(proc => {
                const statusBadge = proc.status === 'new' ? '🆕 NEW' : 
                                   proc.status === 'active' ? '✅ Active' : 
                                   proc.status === 'merge' ? '🔄 Merge' : 
                                   proc.status === 'todo' ? '📋 To implement' : '';
...
function exportOpsRationaleToMarkdown() {
    ...
    markdown += `In modern process management, "One Size Fits All" is a failure mode. Holding a Creative Director to the documentation standards of a Brain Surgeon is not just unfair—it is operationally destructive.\n\n`;
    ...
    markdown += `**Targets:** 40% – 50%\n\n`;
    markdown += `### Focus: Reproducibility and Technical Debt\n\n`;
    markdown += `This is the pivot point. These industries rely on complex intellectual property. If the process isn't documented, the product becomes "fragile"—it breaks when the creator leaves. We document to ensure work is reusable.\n\n`;
